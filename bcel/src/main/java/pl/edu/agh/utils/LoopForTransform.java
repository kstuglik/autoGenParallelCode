...
public class LoopForTransform {
    ...
    public static void addSubTaskMethodFromNestedLoop(
        ...
        int idNameAccessToChangeWithCurrRow = LocalVariableUtils.getIdLVarByName(lvarNameToReplaceWithCurrRow, mgOld);
...
//      CREATE NEW VARIABLES IF NO EXIST AND PREPARE hASH MAP WITH OLD AND NEW ID
        HashMap<Integer, Integer> hashmapIdOldAndNewLVar =
                LocalVariableUtils.prepareLVarIfNoExistedAndReturnHashmap(mgOld, mgNew);
...
//      search array/object load, e.i.: ILOAD & AALOAD
            if (ih1.getInstruction().toString().contains("load")) {
                ...
                int idNew = hashmapIdOldAndNewLVar.getOrDefault(idOld, -1);
                if (idNew != -1) {
                    LocalVariable l = LocalVariableUtils.getLVarById(idOld, mgOld.getLocalVariableTable(mgOld.getConstantPool()));
                    System.out.println("czy znalazlem: " + l.getName() + ", " + l.getIndex() + "," + l.getSignature());


                    if (l.getName().equals("rowNum")) {
                        int idCurrRow = LocalVariableUtils.getIdLVarByName("currRow", mgNew);
...
//      >>>>>>>>>>>>>>>>>>>>>>      NEW INSTRUCTION INSIDE INNER LOOP BODY START HERE   >>>>>>>>>>>>>>>>>>>>>>
        ...
        PreparedFieldsAndMethods.changeBodyInnerLoopForMatrixMultiply(mg, il_new, factory);
...
//
            ...
            if (idNEW != -777) {
                LocalVariable lv = LocalVariableUtils.getLVarById(idNEW, mgNEW.getLocalVariableTable(mgNEW.getConstantPool()));
